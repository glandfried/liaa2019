...
\begin{equation*}
 \text{Bayes factor} = \frac{P(M_1|D)}{P(M_2|D)} = \frac{P(D|M_1)P(M_1)}{P(D|M_2)P(M_2)} \pause = \frac{P(D|M_1)}{\underbrace{P(D|M_2)}_{\text{\scriptsize Evidence!}}}   
\end{equation*}
...
\centering
$P(M_i|D) > P(M_j|D) \Longleftrightarrow P(D|M_i) > P(D|M_j)$
...
Divergence between truth $P(C)=p$ and model $P(C|M_q)=q$ for each event $C_i \in C$, $P(C_i)=p_i$ and $P(C_i|M_q) = q_i$
\begin{equation}
...
\begin{equation}\footnotesize
  D_{KL}(p,q) - D_{KL}(p,r) = \underbrace{\sum_{i=1}^{|C|} p_i \text{log}(r_i)}_{E[\text{log}(r)]} -\underbrace{\sum_{i=1}^{|C|} p_i \text{log}(q_i)}_{E[\text{log}(q)]} 
\end{equation}
...
\end{textblock}
 ...
 \centering{With great dishonesty, great overfiting}
